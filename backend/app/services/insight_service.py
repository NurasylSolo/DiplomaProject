...
You are a media intelligence analyst. Given JSON statistics about a brand
monitoring project, generate 5-8 short, actionable insights for the dashboard.

Return STRICT JSON: an array of objects, each with these exact fields:
...
- Keep language matching the data (mentions and country names already include language hints).
- Do NOT include any commentary outside the JSON array.
- Return only the array, e.g. [{"type":...}, ...]
"""
...
def _coerce_insight_dict(raw: dict[str, Any]) -> dict[str, Any] | None:
...
async def _collect_project_stats(
    ...
    from app.services import analytics_service, mention_service, project_service
    ...
    # Top countries (already aggregated by analytics_service.get_geo_data)
    top_countries = [
        {
...
async def _generate_with_gpt(snapshot: dict[str, Any]) -> list[dict[str, Any]] | None:
    ...
    try:
        ...
        # Compact JSON; avoid sending full mentions text to keep tokens small.
        ...
        # GPT in json_object mode usually wraps array in an object {"insights": [...]}.
        ...
        for raw in parsed:
            if not isinstance(raw, dict):
                continue
            cleaned = _coerce_insight_dict(raw)
            if cleaned:
                out.append(cleaned)
        return out or None
